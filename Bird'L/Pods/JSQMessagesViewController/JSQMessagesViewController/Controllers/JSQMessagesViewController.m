...
- (BOOL)jsq_inputToolbarHasReachedMaximumHeight
{
    return CGRectGetMinY(self.inputToolbar.frame) == (64 + self.topContentAdditionalInset);
}
...
- (void)jsq_adjustInputToolbarForComposerTextViewContentSizeChange:(CGFloat)dy
{
    ...
    if (newToolbarOriginY <= 64 + self.topContentAdditionalInset) {
        dy = toolbarOriginY - (64 + self.topContentAdditionalInset);
        [self jsq_scrollComposerTextViewToBottomAnimated:YES];
    }
...
- (void)jsq_updateCollectionViewInsets
{
    [self jsq_setCollectionViewInsetsTopValue:64 + self.topContentAdditionalInset
